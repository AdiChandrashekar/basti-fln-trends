...
Scope: Grade 2 competency performance in Basti district, Aug 2025 to Aug 2026, from five district assessment tools plus the DiD baseline (Nov 2025) and midline/EOY (Mar 2026). Built by `build_trends.py`. Every definition below is implemented there. Do not re-derive any of it in the dashboard layer.
...
| `data/did_baseline_midline_comparison.csv` | competency | Standalone baseline vs midline comparison. |
...
| `did_midline` | Mar 2026 | 177 (40 schools) | Slide chart top band: the "76-100%" or "More than 75%" band. For ORF it is the 45-59 plus 60+ bands. |

**Midline caveat.** The slide's top band is strictly above 75%. A student at exactly 75% (for example 3 of 4 items) is counted as cleared in the Excel data but not in the midline. The midline therefore understates clearance somewhat relative to the Excel tools. It is flagged `slide_band_gt75_not_ge75`.
...
| Q4_2025 | Jan-Mar 2026 | no | Mar (midline) |
...
- **Midline bands** come from the slides, whose top band is above 75%.
...
- **`metric_basis`** records which kind of number sits in `pct_students_cleared`:
  ...
  - `slide_top_band_gt75`: DiD midline top band.
...
- **`view = mastery_bands`:** band and tier percentages.
  - Sources: student scores (Excel) or slide charts (midline).
  - CPM bands are included for ORF and the midline timed measures.
...
- **Use `mean_change` for any baseline-vs-midline claim.** The series values sit on different bases.
...
4. The DiD baseline enters as % correct, the midline as its above-75% top band, and the Excel tools as clearance rates. Steps between the baseline and anything else are tagged `metric_basis_changed` and must not be drawn as real change.
